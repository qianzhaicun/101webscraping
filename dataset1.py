# -*- coding: utf-8 -*-
"""
Created on Sat Sep  9 09:07:42 2017
...
if __name__ == "__main__":
    ...
    """ 
    """正则
    anum = re.findall('<div class="new-results">([...])</div>',ahtml)
    print(anum)
    """
    """ lxml """
    tree = fromstring(ahtml)
    results = tree.xpath('//div[@class="text-center getstarted"]//small//a//text()')
    results = "".join(results)
    results = results.strip()
    results = results.replace(",","")
    if len(results) > 0:
        results = re.findall('(.*?) datasets',results)[0]
    else:
        results = None
    print(results)
    ...
    ahtml = download(url)
    tree = fromstring(ahtml)
    res = tree.xpath('//h3[@class="dataset-heading"]//a//text()')[0]
    print(res)
    ...
    """
    3. The number of people who visited 
    a U.S. government website using Internet Explorer 6.0 in the last 90 days 
    """
